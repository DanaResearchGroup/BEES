...
from pydantic import BaseModel, conint, confloat, constr, field_validator, ValidationInfo, Field, model_validator
...
class Species(BaseModel):
    ...
    """
    label: str
    concentration: Union[Annotated[float, Field(gt=0)], Tuple[Annotated[float, Field(gt=0)], Annotated[float, Field(gt=0)]]]
    ...
    def validate_molecular_descriptors_exclusivity(self):
        """
        Ensure only one molecular descriptor (smiles, inchi, adjlist) is provided.
        This prevents ambiguity about which descriptor should be used for molecular identification.
        """
        descriptors = []
        if self.smiles:
            descriptors.append('smiles')
        if self.inchi:
            descriptors.append('inchi')
        if self.adjlist:
            descriptors.append('adjlist')
        
        if len(descriptors) > 1:
            raise ValueError(
                f"Only one molecular descriptor can be provided. Found: {descriptors}. "
                f"Please provide either 'smiles', 'inchi', or 'adjlist', but not multiple."
            )
        elif len(descriptors) == 0:
            raise ValueError(
                "At least one molecular descriptor must be provided. "
                "Please provide either 'smiles', 'inchi', or 'adjlist'."
            )
...
class Settings(BaseModel):
    ...
    toleranceKeepInEdge: confloat(gt=0) = 0 #threshold for keeping species in the edge
